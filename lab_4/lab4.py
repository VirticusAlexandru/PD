...
annotation_folder = 'D:\\UTM\\PD\\PD_git_lab\\lab4\\'
...
def create_annotation_file(source_folder, class_name):
    with open('annotations.csv', 'w', newline='') as csv_file:
        ...
        writer.writerow(['D:\\UTM\\PD\\PD_git_lab\\lab_3\\dataset3', '..\\lab_2\\dataset', 'zebra'])
...
class DatasetApp(QtWidgets.QWidget):
    ...
    def initUI(self):
        ...
        self.browse_button = QtWidgets.QPushButton('Выбрать папку  датасетом', self)
    ...
    def browse_dataset_folder(self):
        self.folderpath = QtWidgets.QFileDialog.getExistingDirectory(self, 'Выберите папку  исходным датасетом')
        if self.folderpath:
    ...
    def create_annotation(self):
        if self.folderpath:
            create_annotation_file(self.folderpath, 'zebra')
            create_annotation_file(self.folderpath, 'tulip')
            pass

    def get_next_class1(self):
        if self.folderpath:
            next_instance = get_next_instance('zebra', self.folderpath)
            if next_instance:
